...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Azca persistent cross-session memory.")
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Azca gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Azca's identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Azca asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    ...
    print("\nHoncho migration: OpenClaw native memory → Azca\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Azca persistent memory")
    ...
    if user_files:
        ...
        print("  (Azca calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Azca, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Azca:     cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Azca:     Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Azca:     Honcho observes every message and updates representations")
    ...
    print("    Azca:     per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Azca Honcho",
    )
