...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Azca session")
        ...
        @tree.command(name="status", description="Show Azca session status")
        ...
        @tree.command(name="stop", description="Stop the running Azca agent")
        ...
        @tree.command(name="update", description="Update Azca to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Azca gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start an Azca session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Azca in the thread",
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Azca: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        thread_name = content[:80] if content else "Azca"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Azca: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Azca semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Azca handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Azca to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Azca needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Azca could not create a Discord thread for "
...
def interactive_setup() -> None:
    ...
    print()
    print_info("📬 Home Channel: where Azca delivers cron job results,")
